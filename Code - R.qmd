...
# Creates a table with gene_id and the expected_count of all .genes 
load_and_merge_gene_results <- function(folder, file_labels) {
...
RSEM_counts_refGene <- load_and_merge_gene_results("RSEM_refGene", dataset_labels)
RSEM_counts_ncbiRefSeq <- load_and_merge_gene_results("RSEM_NCBI", dataset_labels)
```
...
# Log transform the data to reduce the range of expression levels
RSEM_counts_log <- log2(RSEM_counts_ncbiRefSeq + 1)  # Add 1 to avoid log(0)

# Calculate variance for each gene and filter based on a threshold
# Adjust threshold based on the level of filtering you want
gene_variances <- apply(RSEM_counts_log, 1, var)
# Further filter genes with higher variance if needed (adjust threshold as appropriate)
high_variance_genes <- RSEM_counts_log[gene_variances > 2, ]  # Increase threshold for more filtering

# Create the clustered heatmap with enhancements
library(pheatmap)
pheatmap(
  as.matrix(high_variance_genes), 
  color = colorRampPalette(c("navy", "white", "firebrick3"))(100),  # Use more color gradients
  cluster_rows = FALSE, 
  cluster_cols = TRUE, 
  ...
  fontsize_row = 6,
  fontsize_col = 8,
  main = "Clustered Heatmap of Transcript Expression Levels (Log-Transformed, Filtered & Scaled)"
)
